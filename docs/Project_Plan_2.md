...
The application is designed to be accessible to small and rural farmers while creating sustainable revenue streams through simplified subscription and consultation services.

### Revenue Streams

**Platform Subscription (Both Sides):**
- **$5.00/month per user** - "Pay to play" platform access fee
- **Farmers:** Unlimited health records + ability to request veterinary consultations
- **Veterinarians:** Access to consultation queue + ability to accept consultations
- No free tier to maintain quality and prevent abuse
- Simple, predictable recurring revenue model

**Consultation Services:**
- **$25 flat rate per consultation** (simple, predictable pricing)
- Covers up to 10 animals with same health issue
- 24-hour interaction window with up to 10 messages
- Photo/video review + async Q&A + video escalation if needed
- **Platform fee:** 10% ($2.50 per consult)
- **Veterinarian receives:** 90% ($22.50 per consult)

**Billing Structure:**
- Farmer pays: $5/mo subscription + $25 per consultation
- Veterinarian pays: $5/mo subscription
- Veterinarian earns: $22.50 per consult (~$90/hour for 15-20 min work)

**Value Proposition:**
- **Farmers:** Access to specialty vets in hours (not days) for fraction of farm visit cost
- **Veterinarians:** New revenue stream, flexible scheduling, no travel overhead, earn during downtime
- **Platform:** Dual revenue model (recurring subscriptions + transaction fees on consults)

**Key Advantages:**
- Much simpler than complex tiered pricing models
- Predictable costs for farmers (no surprise fees)
- Attractive economics for vets ($22.50 for 15-20 min consultation)
- Platform earns recurring + transaction revenue
...
- **Subscription**: Each user pays $5/month for platform access
...
- **Provider Consortium Access (MVP FEATURE - Q1 2026)**:
  - **Photo-first consultation workflow**: Upload photos/videos of animal with description
  - **Specialty vet queue**: Browse available providers by specialty (internal medicine, nutrition, dermatology) and language
  - **Fast response times**: 2-hour response (monitored queue) or 48-hour maximum
  - **Asynchronous messaging**: Q&A with vet for clarification
  - **Video escalation**: Vet can upgrade to live video if needed (included in $25 fee)
  - **Simple pricing**: $25 flat rate per consultation (covers up to 10 animals with same issue)
  - **24-hour consult window**: Up to 10 messages within 24 hours from vet's first response
  - **Treatment plans**: Vet recommendations automatically saved to animal health record
  - **Cross-timezone coverage**: Access to vets across time zones
  - **Consultation history**: Track all past consults and outcomes
  - **SLA guarantee**: Consultation coupon if SLA violated
...
- **User Management** (Owner and Manager roles):
  ...
  - Each user requires $5/month subscription to access platform
  - View all users and their farm assignments

- **Subscription & Billing Management** (Owner only):
  - View current subscription status ($5/month per user)
  - Manage payment method
  - View billing history
  - View consultation usage and charges ($25 per consult)
  - Track platform fees (10% on consultations)
  - Simple billing: $5/mo subscription + $25 per consultation used
...
- **Telehealth Consultation System (MVP FEATURE - Q1 2026)**:

  **Core Consultation Workflow:**
  - Photo-first approach: Farmer uploads photos/videos of animal with description
  - Available vets notified via push, text, and email
  - Vet responds within 2 hours (monitored queue) or 48 hours (unmonitored)
  - Asynchronous Q&A messaging for clarification
  - Video escalation if vet deems necessary (included in consultation fee)
  - Treatment plan and notes automatically saved to animal health record

  **Consultation Structure:**
  - **$25 flat rate** per consultation (simple, predictable)
  - Covers **up to 10 animals** with same health issue (herd-level assessment)
  - **24-hour interaction window** from vet's first response
  - **Up to 10 messages total** (prevents endless back-and-forth)
  - **Video included** if vet escalates (no additional charge)
  - **Clear boundaries:** One health issue, one consult period

  **New Consult Required For:**
  - Different health issue (even same animal)
  - Follow-up after 24-hour window closes
  - New symptoms that emerge
  - Different animals with different issues

  **Provider Queue Management:**
  - Vets opt-in to queue when available
  - Queue shows: Number available by specialty + languages (self-reported)
  - Cross-timezone coverage (Eastern vets serve Pacific farmers, etc.)
  - Push + text + email notifications for new consult requests

  **Service Level Agreement:**
  - 2-hour response when queue monitored
  - 48-hour maximum when limited availability
  - Vets accept SLA when joining queue
  - SLA violations = farmer receives coupon for future consult

  **Provider Credentialing (Simplified for Vet Med):**
  - State veterinary license number required
  - License validation during signup (spot-check against state boards)
  - Self-reported specialties (internal medicine, nutrition, dermatology)
  - Self-reported multi-lingual capabilities
  - Annual re-verification of license status
  - **NO diploma uploads** (too heavy for vet med, not HIPAA-regulated)

  **Provider Types (Initial Launch):**
  - Veterinarians - Internal Medicine specialists
  - Animal Nutritionists
  - Dermatology specialists
  - (Expansion to other specialties based on demand)

  **Pricing Model (Simplified):**
  - Provider subscription: $5/month (pay to play)
  - Platform fee: 10% on consultations ($2.50 per $25 consult)
  - Provider receives: $22.50 per consult (~$90/hour rate)
  - No complex tiered pricing or premium add-ons

  **Abuse Prevention:**
  - 24-hour consult lifetime (hard cutoff)
  - 10-message limit within window
  - Clear scope definition (one issue)
  - Vet can close consult early if resolved quickly
  - Rating system flags problem users on both sides

  **Payment Processing:**
  - Stripe integration for consultation fees
  - Automatic 90/10 split (vet/platform)
  - Monthly provider subscription billing
  - Farmer charged $25 per consult at time of request

- **Enhanced Provider Features (Post-MVP)**:

  **Rating & Review System:**
  - Farmers rate consultations after completion
  - Vets rate farmer interactions
  - Public ratings visible when choosing providers
  - Quality control triggers for low-rated providers

  **Favorite Providers:**
  - Farmers can mark providers as favorites
  - Direct request option to preferred vets
  - Builds continuity of care relationships
  - Providers see favorite count

  **Provider Tools:**
  - Consultation history and earnings dashboard
  - Schedule management and availability settings
  - Customer relationship management (repeat clients)
  - Private notes per client/animal (provider-only)
